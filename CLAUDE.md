...
- **Branch/remote metadata** stored in git config: `branch.<name>.created-by`, `branch.<name>.pr-url`, `remote.<name>.created-by`. Used by `_cleanup_utpr_remote` to safely remove only utpr-created remotes.
